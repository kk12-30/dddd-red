...
var (
	...
	nucleiInitialized = false  // 是否已初始化
	initialized sync.Mutex     // 用于Nuclei初始化的互斥锁
)
...
func CallNuclei(param NucleiParams) []output.ResultEvent {
	...
	// 如果之前已初始化过，则先重置HTTP相关资源
	if nucleiInitialized {
		gologger.Info().Msg("重置Nuclei运行状态，清理资源")
		
		// 重置统计、监听器等资源
		output.Results = []output.ResultEvent{}
	...
	for target, pocs := range param.TargetAndPocsName {
		for i, poc := range pocs {
			if strings.Contains(poc, ":\\") || strings.Contains(poc, ":/") {
				// 对于绝对路径的POC，确保路径分隔符一致
				normalizedPath := strings.ReplaceAll(poc, "\\", "/")
				param.TargetAndPocsName[target][i] = normalizedPath
				...
				if !found {
					customTemplates = append(customTemplates, normalizedPath)
				}
			}
		}
	}
	...
	if len(customTemplates) > 0 {
		gologger.Info().Msgf("添加 %d 个自定义POC模板", len(customTemplates))
		options.Templates = append(options.Templates, customTemplates...)
	}
